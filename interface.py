...
def traduzir_para_python(codigo):
    ...
    for i in range(len(linhas)):
        # Substituir palavras reservadas usando regex para garantir que sejam isoladas
        for chave, valor in substituicoes.items():
            padrao = r'\b' + re.escape(chave) + r'\b'
            linhas[i] = re.sub(padrao, valor, linhas[i]).strip()

        # Substituir chaves delimitadoras de blocos
        # `{` para `:` no final da linha e `}` para string vazia
        linhas[i] = re.sub(r'\{\s*$', ':', linhas[i])
        linhas[i] = re.sub(r'\}', '', linhas[i])
        linhas[i] = re.sub(r'\s+', ' ', linhas[i])
